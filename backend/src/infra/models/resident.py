...
class Resident(SQLModel, table=True):
    __tablename__ = "resident" # type: ignore
    resident_id: str = Field(default=None,
                             primary_key=True,
                             nullable=False,
                             unique=True)
    ...
    status_payment: bool = Field(default=False, nullable=False)
    admin_id: str = Field(default=None,
                          sa_column=Column(
                              String,
                              ForeignKey("admin.admin_id",
                                         ondelete="CASCADE",
                                         onupdate="CASCADE")))
    created_at: date.datetime = Field(default_factory=date.datetime.utcnow,
                                      nullable=False)
